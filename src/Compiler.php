...
class Compiler extends Engine
{
	...
	$search = "~{$ldq}\*(.*?)\*{$rdq}"
		. "|{$ldq}\s*literal\s*{$rdq}(.*?){$ldq}\s*/literal\s*{$rdq}"
		. "|{$ldq}\s*php\s*{$rdq}(.*?){$ldq}\s*/php\s*{$rdq}~s";
	...
	if ($start_tag) {
	    ...
	    $output .= "\$this->_tag_stack[] = array('$tag_command', array(".implode(',', $arg_list).')); ';
	...
	if(!empty($this->_reg_objects[$object][1]) && !in_array($obj_comp, $this->_reg_objects[$object][1]))
	{
	    throw new BadSyntaxException(
	...
	* {@deprecated}
	*/
	function _push_cacheable_state()
	{
		return '';
	}
